---
title: "Project Cryptocurrency"
author: "Abi Alfan, 123190144 123190120"
date: "28/11/2021"
...
```{r}
tw = searchTwitter('cryptocurrency', 
                   n = 1000,
                   retryOnRateLimit = 10e5, lang = "id") #retryOnRateLimit untuk looping
saveRDS(tw,file = 'new-tweet.rds')
```

```{r}
tw <- readRDS('new-tweet.rds')
...
View(dataframe)
write.csv(dataframe,'tweet_cryptocurrency.csv')
```
...
cryptocurrency_dataset <-read.csv("tweet_cryptocurrency.csv",stringsAsFactors = FALSE)

#digunakan untuk mengeset variabel cloumn text menjadi char
review <- as.character(cryptocurrency_dataset$text)
...
review_combine<-cbind(cryptocurrency_dataset$text,s)
par(mar=rep(3,4))
...
df<-read.csv("tweet_cryptocurrency.csv",stringsAsFactors = FALSE) #membaca file CSV
...
#klasifikasi dari data cryptocurrency dengan melakukan data training dan data test
df.train<-df[1:589,]
df.test<-df[590:1177,]
dtm.train<-dtm[1:589,]
...
## Shiny
#membuka file csv
twitter <- read.csv(file="tweet_cryptocurrency.csv",header=TRUE)
...
ui <- fluidPage(
  titlePanel("Penggunaan Kata Cryptocurrency Pada Twitter"), #halaman judul dr fluid page
...
server <- function(input, output) {
  ...
  #Barplot
  output$scatterplot <- renderPlot({vaksin_dataset<-read.csv("tweet_cryptocurrency.csv",stringsAsFactors = FALSE)
  review <-as.character(cryptocurrency_dataset$text)
  get_nrc_sentiment('happy')
  get_nrc_sentiment('excitement')
  s<-get_nrc_sentiment(review)
